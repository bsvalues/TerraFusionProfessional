/**
 * Data Processor for Benton County Property Data
 * 
 * This script processes CSV files from the Benton County Assessor's Office
...
function processPropertyData() {
  ...
  const comperData = readCsvFile(comperFile);
  const propertyValData = readCsvFile(propertyValFile);
  const addressData = readCsvFile(addressFile);
  const landDetailData = readCsvFile(landDetailFile);
  const legalDescData = readCsvFile(legalDescFile);
  ...
  addressData.forEach(record => {
    ...
    // This is a simplified match - in real data we'd need a more robust approach
    // to match addresses to properties since the acct_id isn't the prop_id
...
function processHistoricalData(properties) {
  const rollValueHistoryData = readCsvFile(rollValueHistoryFile);
...
function processSalesData(properties) {
  const salesData = readCsvFile(salesChangeOwnerFile);
...
function processPermitsData() {
  const permitsData = readCsvFile(permitsFile);
