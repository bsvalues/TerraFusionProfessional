...
const ResultVisualizer: React.FC<ResultVisualizerProps> = ({ data }) => {
  ...
  const analyzDataForVisualization = (data: any) => {
    ...
    // Check for comparative market analysis
    if (data && data.comparables && Array.isArray(data.comparables) && data.marketAnalysis) {
      // Add a bar chart comparing the target property to similar properties
      visualizations.push({
        type: 'barChart',
        title: 'Comparable Properties Analysis',
        description: 'Value comparison between the target property and similar nearby properties',
        data: [
          {
            name: 'Target Property',
            value: data.targetProperty.value
          },
          {
            name: 'Average Comp',
            value: parseFloat(data.marketAnalysis.avgValue)
          },
          {
            name: 'Median Comp',
            value: parseFloat(data.marketAnalysis.medianValue)
          },
          {
            name: 'Min Comp',
            value: parseFloat(data.marketAnalysis.minValue)
          },
          {
            name: 'Max Comp',
            value: parseFloat(data.marketAnalysis.maxValue)
          }
        ]
      });
      
      // Add a table showing the comparable properties
      visualizations.push({
        type: 'table',
        title: 'Comparable Properties',
        description: `${data.comparables.length} similar properties near ${data.targetProperty.address}`,
        data: data.comparables.map((comp: any) => ({
      ...
      // Add detail card for market analysis
      visualizations.push({
        type: 'marketAnalysis',
        title: 'Market Analysis',
        description: 'Detailed analysis of the subject property market position',
        data: {
  ...
  const renderVisualization = (viz: any) => {
    ...
    switch (viz.type) {
      ...
      case 'marketAnalysis': {
        // Display a detailed card with information about the target property and market analysis
        const { targetProperty, marketAnalysis } = viz.data;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card className="border border-border">
                <CardHeader className="bg-primary/5">
                  <CardTitle className="text-base">Target Property Details</CardTitle>
                </CardHeader>
                <CardContent className="p-4 space-y-3">
                  <div>
                    <p className="text-sm font-medium">Address</p>
                    <p className="text-sm text-muted-foreground">{targetProperty.address}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <p className="text-sm font-medium">Value</p>
                      <p className="text-sm text-muted-foreground">{formatCurrency(targetProperty.value)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">Square Feet</p>
                      <p className="text-sm text-muted-foreground">{targetProperty.squareFeet}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">Price per Sq.Ft.</p>
                      <p className="text-sm text-muted-foreground">{targetProperty.pricePerSqFt !== 'N/A' 
                        ? formatCurrency(parseFloat(targetProperty.pricePerSqFt)) 
                        : 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">Type</p>
                      <p className="text-sm text-muted-foreground">{targetProperty.propertyType || 'N/A'}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{formatCurrency(parseFloat(marketAnalysis.avgValue))}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">Median Value</p>
                      <p className="text-sm text-muted-foreground">{formatCurrency(parseFloat(marketAnalysis.medianValue))}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">Min Value</p>
                      <p className="text-sm text-muted-foreground">{formatCurrency(parseFloat(marketAnalysis.minValue))}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium">Max Value</p>
                      <p className="text-sm text-muted-foreground">{formatCurrency(parseFloat(marketAnalysis.maxValue))}</p>
                    </div>
                  </div>
                  
                  <div>
                    <p className="text-sm font-medium">Price per Sq.Ft. in Market</p>
                    <p className="text-sm text-muted-foreground">{formatCurrency(parseFloat(marketAnalysis.avgPricePerSqFt))}</p>
                  </div>
                  
                  <div>
                    <p className="text-sm font-medium">Value Percentile</p>
                    <div className="w-full bg-muted h-3 rounded-full overflow-hidden mt-1">
                      <div 
                        className="bg-primary h-full" 
                        style={{ width: `${marketAnalysis.valuePercentile}%` }}
                      ></div>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      The property is in the {marketAnalysis.valuePercentile}th percentile of comparable properties
                    </p>
                  </div>
                  
                  <div>
                    <p className="text-sm font-medium">Recommended Value Range</p>
                    <p className="text-sm text-muted-foreground">
                      {formatCurrency(parseFloat(marketAnalysis.recommendedValueRange.low))} - {formatCurrency(parseFloat(marketAnalysis.recommendedValueRange.high))}
                    </p>
                  </div>
                </CardContent>
              </Card>
